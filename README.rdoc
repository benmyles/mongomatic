...
Observer's following the convention, CollectionNameObserver < Mongomatic::Observer will automatically be added to the CollectionName class if it includes Mongomatic::Observable. Unlike ActiveRecord the existence of a CollectionNameObserver will not automatically load the CollectionName class. Instead Mongomatic handles both the case where the CollectionName class is loaded before and after the CollectionNameObserver.
...
You can add your own observers to CollectionName using CollectionName.add_observer(klass).
  ...
  Person.add_observer(MyCustomCallbacks)

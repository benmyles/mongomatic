...
Observer's following the convention, CollectionNameObserver < Mongomatic::Observer will automatically be added to the CollectionName class if it includes Mongomatic::Observable. Unlike ActiveRecord the existing of a CollectionNameObserver will not automatically load the CollectionName class. Instead Mongomatic handles both the case where the CollectionName class is loaded before and after the CollectionNameObserver.
